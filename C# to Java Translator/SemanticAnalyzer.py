...
class Scope:
    ...
    def add_variable(self, name: Token, type: NodeType | str, value=None) -> None:
        if name.value in self.variable_table:
            raise SemanticAnalyzerException(f"Id has already declared : line {name.lineno}, pos {name.pos}")
    ...
    def update_variable(self, name: Token, value) -> None:
        ...
        if variable is None:
            raise SemanticAnalyzerException(f"Update a undeclared variable : line {name.lineno}, pos {name.pos}")
        ...
        match node:
            ...
            case NodeVar():
                if not scope.has_variable(node.id):
                    raise SemanticAnalyzerException(f"Id is not declared : line {node.id.lineno}, pos {node.id.pos}")
                else:
